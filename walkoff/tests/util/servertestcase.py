...
from walkoff.tests.util.mock_objects import *
from walkoff.tests.util.thread_control import *
...
class ServerTestCase(unittest.TestCase):
    test_workflows_path = walkoff.tests.config.test_workflows_path_with_generated
    patch = True
    ...
    def setUpClass(cls):
        ...
        if (walkoff.tests.config.test_data_dir_name not in os.listdir(walkoff.tests.config.test_path)
            or os.path.isfile(walkoff.tests.config.test_data_path)):
            if os.path.isfile(walkoff.tests.config.test_data_path):
                os.remove(walkoff.tests.config.test_data_path)
            os.makedirs(walkoff.tests.config.test_data_path)
        walkoff.appgateway.cache_apps(path=walkoff.tests.config.test_apps_path)
    ...
    def tearDownClass(cls):
        if walkoff.tests.config.test_data_path in os.listdir(walkoff.tests.config.test_path):
            if os.path.isfile(walkoff.tests.config.test_data_path):
                os.remove(walkoff.tests.config.test_data_path)
            else:
                shutil.rmtree(walkoff.tests.config.test_data_path)
        walkoff.appgateway.clear_cache()
        walkoff.server.flaskserver.running_context.controller.shutdown_pool()

    def setUp(self):
        walkoff.config.paths.workflows_path = walkoff.tests.config.test_workflows_path_with_generated
        walkoff.config.paths.installed_apps_path = walkoff.tests.config.test_apps_path
        walkoff.config.paths.default_appdevice_export_path = walkoff.tests.config.test_appdevice_backup
        walkoff.config.paths.default_case_export_path = walkoff.tests.config.test_cases_backup
        if os.path.exists(walkoff.tests.config.test_workflows_backup_path):
            shutil.rmtree(walkoff.tests.config.test_workflows_backup_path)

        shutil.copytree(walkoff.config.paths.workflows_path, walkoff.tests.config.test_workflows_backup_path)
    ...
    def tearDown(self):
        ...
        shutil.copytree(walkoff.tests.config.test_workflows_backup_path, walkoff.config.paths.workflows_path)
        shutil.rmtree(walkoff.tests.config.test_workflows_backup_path)
        for data_file in os.listdir(walkoff.tests.config.test_data_path):
            try:
                os.remove(os.path.join(walkoff.tests.config.test_data_path, data_file))
            except WindowsError as we:  # Windows sometimes makes files read only when created
                os.chmod(os.path.join(walkoff.tests.config.test_data_path, data_file), stat.S_IWRITE)
                os.remove(os.path.join(walkoff.tests.config.test_data_path, data_file))
