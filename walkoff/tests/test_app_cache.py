...
class TestAppCache(TestCase):
    ...
    def test_get_qualified_function_name(self):
        self.assertEqual(_get_qualified_function_name(f1), 'walkoff.tests.test_app_cache.f1')

    def test_get_qualified_function_name_with_class(self):
        def f1(): pass

        self.assertEqual(_get_qualified_function_name(f1, cls=TestAppCache),
                         'walkoff.tests.test_app_cache.TestAppCache.f1')

    def test_get_qualified_class_name(self):
        self.assertEqual(_get_qualified_class_name(TestAppCache),
                         'walkoff.tests.test_app_cache.TestAppCache')

    def test_strip_base_module_from_qualified_name(self):
        self.assertEqual(_strip_base_module_from_qualified_name('walkoff.tests.test_app_cache.f1', 'walkoff.tests'),
                         'test_app_cache.f1')

    def test_strip_base_module_from_qualified_name_invalid_base_module(self):
        self.assertEqual(_strip_base_module_from_qualified_name('walkoff.tests.test_app_cache.f1', 'invalid'),
                         'walkoff.tests.test_app_cache.f1')

    def cache_app(self, A):
        self.cache._cache_app(A, 'A', 'walkoff.tests.test_app_cache')

    def test_cache_module(self):
        module = import_module('walkoff.tests.testapps.HelloWorldBounded.main')
        from walkoff.tests.testapps.HelloWorldBounded.main import Main
        self.cache._cache_module(module, 'HelloWorldBounded', 'walkoff.tests.testapps')
    ...
    def test_cache_module_nothing_found(self):
        module = import_module('walkoff.tests.testapps.HelloWorldBounded.display')
        self.cache._cache_module(module, 'HelloWorldBounded', 'walkoff.tests.testapps')
        self.assertDictEqual(self.cache._cache, {})

    def test_cache_module_no_class(self):
        module = import_module('walkoff.tests.testapps.HelloWorldBounded.actions')
        self.cache._cache_module(module, 'HelloWorldBounded', 'walkoff.tests.testapps')
    ...
    def test_import_and_cache_submodules_from_string(self):
        self.cache._import_and_cache_submodules('walkoff.tests.testapps.HelloWorldBounded', 'HelloWorldBounded',
                                                'walkoff.tests.testapps')
        from walkoff.tests.testapps.HelloWorldBounded.main import Main
        self.assert_cache_has_main(Main, app='HelloWorldBounded')
        expected = {
            'main.Main.helloWorld', 'main.Main.repeatBackToMe', 'main.Main.returnPlusOne', 'main.Main.pause',
    ...
    def test_import_and_cache_submodules_from_module(self):
        module = import_module('walkoff.tests.testapps.HelloWorldBounded')
        self.cache._import_and_cache_submodules(module, 'HelloWorldBounded', 'walkoff.tests.testapps')
        from walkoff.tests.testapps.HelloWorldBounded.main import Main
        self.assert_cache_has_main(Main, app='HelloWorldBounded')
        expected = {
            'main.Main.helloWorld', 'main.Main.repeatBackToMe', 'main.Main.returnPlusOne', 'main.Main.pause',
    ...
    def test_cache_apps(self):
        self.cache.cache_apps(os.path.join('.', 'walkoff', 'tests', 'testapps'))
        from walkoff.tests.testapps.HelloWorldBounded.main import Main
        from walkoff.tests.testapps.DailyQuote.main import Main as DailyMain
        self.assert_cache_has_main(Main, app='HelloWorldBounded')
    ...
    def test_get_all_app_names(self):
        ...
        self.cache_app(A)
        self.cache._cache_app(B, 'B', 'walkoff.tests.test_app_cache.TestAppCache')
    ...
    def test_get_app(self):
        ...
        self.cache_app(A)
        self.cache._cache_app(B, 'B', 'walkoff.tests.test_app_cache')
    ...
    def test_is_app_action_bound_false(self):
        ...
        self.cache_app(A)
        self.cache._cache_app(B, 'B', 'walkoff.tests.test_app_cache')
        self.cache._cache['A'].cache_functions([(xx, self.action_tag)], 'walkoff.tests.test_app_cache')
